...
export default function GlobalScheduler({ isOpen, onClose }: Props) {
  ...
  const typeOptions = useMemo(() => ([
    ...
    { value: "payment", label: "Employee Payment" },
  ]), []);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-xl bg-white rounded-xl border border-gray-200 p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 gap-3 mb-4">
          <Input id="event-title" label="Title" value={title} onChange={setTitle} placeholder="Pay wages, Irrigate North Plot..." required />
          <Input id="event-type" label="Type" variant="select" value={type} onChange={setType} options={typeOptions} />
          <Input id="event-date" label="Date" variant="date" value={date} onChange={setDate} required />
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2">
                ...
                />
                <span className="text-gray-700">No specific target</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
          ...
          {target === "farmland" && (
            <div className="flex flex-col gap-2">
              <label className="text-gray-900 font-medium">Farmland</label>
          ...
          {target === "employee" && (
            <div className="flex flex-col gap-2">
              <label className="text-gray-900 font-medium">Employee</label>
              <div className="max-h-40 overflow-auto pr-2 space-y-2">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={saveEvent} disabled={!canSave}>Save</Button>
        </div>
      </div>
    </div>
  );
